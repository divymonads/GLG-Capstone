...
import flask
import pandas as pd
from model_handler import NERWrapper
...
def ping():
    ...
    try:
        ...
        if not NERWrapper.initialized:
            NERWrapper.initialize()
        health = NERWrapper.initialized
    except Exception as e:
        print("error occurred loading NER")
        print(e)
        print(os.listdir('/opt/ml/model'))
        health = False
...
def transformation():
    ...
    print("Prediction on:\n", data)
    # Do the prediction
    prediction = NERWrapper.handle(data)
